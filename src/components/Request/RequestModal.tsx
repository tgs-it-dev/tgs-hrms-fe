...
function RequestModal({ open, onClose, title, initialData }: { open: boolean, onClose: () => void, title: string, initialData?: LeaveRequest | null }) {
    ...
    return (
        <AppFormModal
            ...
            submitLabel={initialData ? getLabel('Save Changes', 'حفظ التغييرات') : getLabel('Submit Request', 'إرسال الطلب')}
        ...
        >
            <Box sx={{
                ...
                <AppDropdown
                    ...
                    options={[
                        { value: 'wfh', label: getLabel('Work From Home', 'العمل من المنزل') },
                ...
                {reqType === 'wfh' && (
                    <Box sx={{
                        ...
                        <Typography sx={{
                            ...
                            {getLabel('Your availability during WFH', 'تواجدك خلال العمل من المنزل')}
                        </Typography>
                        <Typography sx={{
                            fontSize: '13px',
                            color: theme.palette.text.primary,
